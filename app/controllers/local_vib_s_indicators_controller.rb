class LocalVibSIndicatorsController < ApplicationController
  before_action :set_local_vib_s_indicator, only: [:show, :edit, :update, :destroy]

  # GET /local_vib_s_indicators
  # GET /local_vib_s_indicators.json
  def index
    @local_vib_s_indicators = LocalVibSIndicator.all
  end

  # GET /local_vib_s_indicators/1
  # GET /local_vib_s_indicators/1.json
  def show
  ...
  def create_vibration(params)
    @local = LocalVibSIndicator.create!(terms: params[:terms],
    ...
    @local.save!
    @x_values = BoostVibLsInd.create!(       frequency_8: params[:values_x][:frequency_8_x],
                                            ...
                                            local_vib_s_indicator_id: @local.id)

    @y_values = BoostVibLsInd.create!(       frequency_8: params[:values_y][:frequency_8_y],
    ...
    @z_values = BoostVibLsInd.create!(       frequency_8: params[:values_z][:frequency_8_z],
  ...
  # POST /local_vib_s_indicators
  # POST /local_vib_s_indicators.json
  def create
    @local_vib_s_indicator = LocalVibSIndicator.new(local_vib_s_indicator_params)

    respond_to do |format|
      if @local_vib_s_indicator.save
        format.html { redirect_to @local_vib_s_indicator, notice: 'Local vib s indicator was successfully created.' }
        format.json { render :show, status: :created, location: @local_vib_s_indicator }
      else
        format.html { render :new }
        format.json { render json: @local_vib_s_indicator.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /local_vib_s_indicators/1
  # PATCH/PUT /local_vib_s_indicators/1.json
  def update
    respond_to do |format|
      if @local_vib_s_indicator.update(local_vib_s_indicator_params)
        format.html { redirect_to @local_vib_s_indicator, notice: 'Local vib s indicator was successfully updated.' }
        format.json { render :show, status: :ok, location: @local_vib_s_indicator }
      else
  ...
  # DELETE /local_vib_s_indicators/1
  # DELETE /local_vib_s_indicators/1.json
  def destroy
    @local_vib_s_indicator.destroy
    respond_to do |format|
      format.html { redirect_to local_vib_s_indicators_url, notice: 'Local vib s indicator was successfully destroyed.' }
  ...
  private
    ...
    def set_local_vib_s_indicator
      @local_vib_s_indicator = LocalVibSIndicator.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def local_vib_s_indicator_params
      params.require(:local_vib_s_indicator).permit(:terms,
                                                    :boost_level_x,
                                                    :boost_level_y,
                                                    :boost_level_z,
                                                    :staff_worker_id,
